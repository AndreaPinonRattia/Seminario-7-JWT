...
export async function updateUser(req: Request, res: Response): Promise<Response> {
    try {
        ...
        // El ID del usuario que está siendo modificado
        const id = req.params.id;

        // Obtenemos los datos actualizados del body de la solicitud
        const { username, name, email } = req.body;

        // Solo permitimos la actualización de los siguientes campos
        const updatedUser = {
            ...(username && { username }),
            ...(name && { name }),
            ...(email && { email })
